...
function AddAuction({ apkid, iscompleted, onRefresh }) {
    ...
    const handleAddDetail = () => {
        // if (ISCOMPLETED_ != true) {
        //     Swal.fire({
        //         icon: "error",
        //         title: "बदल करता येणार नाही.",
        //         text: "शेतकऱ्याचे बिल तयार झाल्यामुळे माहितीमध्ये बदल करता येणार नाही.",
    ...
    const saveGateEntryData = async () => {
        const Newtoken = isTokenGenerated ? GUID : dpkid;
    ...
    // Rollback Master Delete (unchanged)
    const rollbackDelete = async (apkid) => {
        try {
            ...
            await axios.post(`${baseUrl.Url}/backend/api/SP_DeleteAuctionshed`, JSON.stringify(payload), { headers });
            await axios.post(`${baseUrl.Url}/backend/api/SP_DeleteRECEIPTMasterAuction`, JSON.stringify(payload), { headers });
            await axios.post(`${baseUrl.Url}/backend/api/SP_DeleteVyapariMasterAuction`, JSON.stringify(payload), { headers });
    ...
    const handleFormSubmission = async () => {
        ...
        try {
            ...
            const payload1 = {
                apkid: formData.apkid ? formData.apkid : GUID,
            ...
            receiptDetailsPayload = rows.map((row) => {
            ...
            const response4 = await axios.post(`${baseUrl.Url}/backend/api/SP_AddUpdRECEIPTDetails`, receiptDetailsPayload, { headers });
            ...
            if (apkidToRollback) await rollbackDelete(apkidToRollback);

            // Mark as deleted
            auctionShedDetailPayload.forEach(row => row.isdeleted = true);
            receiptDetailsPayload.forEach(row => row.isdeleted = true);
            vyapariBillDetailPayload.forEach(row => row.isdeleted = true);


            // Try to save with isdeleted = true
            try {
                await axios.post(`${baseUrl.Url}/backend/api/SP_AddUpdAuctionShedDeatil`, auctionShedDetailPayload, { headers });
                await axios.post(`${baseUrl.Url}/backend/api/SP_AddUpdRECEIPTDetails`, receiptDetailsPayload, { headers });
                await axios.post(`${baseUrl.Url}/backend/api/SP_AddUpdVyapariBillDetail`, vyapariBillDetailPayload, { headers });
            } catch (e) {
    ...
    useEffect(() => {
        if (formData?.TOKEN) {
            handleSearch(formData.TOKEN); // Auto trigger token search
        }
    }, [formData.TOKEN]);
    ...
    //         const response = await axios.post(
    //             `${baseUrl.Url}/backend/api/GETNewGenrateBillNo`,
    //             payload,
    //             { headers }
    //         );
    ...
    //         setFormData(prevFormData => ({
    //             ...prevFormData,
    //             BILLNO: currentCount + 1
    //         }));

    //         console.log('Generated new Bill number:', currentCount + 1);
    ...
    const closeModal = () => {
        ...
        if (window.history.state === "modal-open") {
            // Remove the modal state so back navigation continues normally
            history.back();
        }
    ...
    //         // Remove body classes and reset overflow
    //         document.body.classList.remove("modal-open");
    //         document.body.style.overflow = "auto";
    //         document.body.style.paddingRight = "";
    //     }
    ...
    const handleInputChangetoken = async (e) => {
        ...
        //         await handleSearchtoken(value);
        //     } else {
        //         await handleSearch('');
        //         await handleSearchtoken('');
        //     }
        // }
    };
    ...
    const handleInputChangejali = (e) => {
        ...
        const totalAfterUpdate = jaliValue;
        const carets = (parseInt(JALI) + 10).toString();
        if (totalAfterUpdate > carets) {
            const remainingJali = carets - totalJaliUsed;

            Swal.fire({
                icon: "error",
                title: "त्रुटी",
                text: `तुम्ही आणखी फक्त ${remainingJali} जाळ्या टाकू शकता.`,
                confirmButtonText: "ठीक आहे",
                allowOutsideClick: false,
                allowEscapeKey: false
            });
    ...
    const handleWeightChange = (e) => {
        ...
        setDetailData((prevData) => ({
            ...prevData,
            weight: inputValue, // keep it as string to allow empty input
                                        ...
                                        {/* <div className="col-lg-2 col-md-6 col-sm-6 col-6">
                                            <div className="mb-0">
                                                ...
                                                <input
                                                    ...
                                                    onKeyDown={(e) => handleKeyDown(e, FARMERNAMERef)}
                                                    readOnly
                                                    style={{
                                                        fontWeight: "900",
                                                        border: "2px solid #ff9800",
                                        ...
                                        <div className="col-lg-3 col-md-4 col-sm-12">
                                            <div className="mb-0">
                                                ...
                                                <Select
                                                    ...
                                                    options={farmer}
                                                    value={farmer.find(option => option.value === formData.MAID) || null}
                                                    onChange={(selectedOption) => {
                                                        const selectedMaid = selectedOption ? selectedOption.value : '';
                                            ...
                                            <div className="col-lg-3 col-md-6 col-sm-12">
                                                <div className="mb-0">
                                                    ...
                                                    <Select
                                                        ...
                                                        openMenuOnFocus={true}
                                                        options={vyapari}
                                                        // onClick={() => { handleSearchtoken(formData.TOKEN) }}
