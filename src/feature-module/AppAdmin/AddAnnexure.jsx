...
const AddAnnexure = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                ...
                setFormData((prev) => ({ ...prev, ALimage: finalFileName }));
            } catch (error) {
                setFilePreviewUrl(null);
                setFormData((prev) => ({ ...prev, ALimage: "" }));
            } finally {
    ...
    const handleFormSubmission = async () => {
        try {
            ...
            const response = await axios.post(
                `${baseUrl.Url}/api/SP_UpdAnnexureFile`,
                payload,
                { headers }
            );
    ...
    const checkFormValidity = (e) => {
        const { Atitle, ALimage, isactive } = formData;
        if (!Atitle) {
            Swal.fire({
                icon: "error",
    ...
    useEffect(() => {
        ...
        const fetchMasterData = async () => {
            try {
                ...
                const response = await axios.post(
                    `${baseUrl.Url}/api/GET_AnnexureFile`,
                    payload,
                    { headers }
                );
                ...
                setFormData({
                    aid: apiData.aid || "",
                    Atitle: apiData.atitle || "",
                    ALimage: apiData.aimage || "",
                    Description: apiData.description || "",
                    isactive: String(apiData.isactive || ""),
                });
                if (apiData.ALimage) {
                    const fileExtension = apiData.ALimage.split('.').pop().toLowerCase();
                    const fileUrl = fileExtension === 'pdf'
                        ? `${baseUrl.Url}/Assets/${apiData.ALimage}`
                        : `${baseUrl.Url}/Images/${apiData.ALimage}`;
                    setFilePreviewUrl(fileUrl);
                }
            } catch (error) {
                ...
                <div className="page-header">
                    <div className="add-item d-flex">
                        <div className="page-title">
                            <h3>{aid ? "ॲनेक्स्चर फाइल" : "नवीन ॲनेक्स्चर फाइल"}</h3>
                            <h6>{aid ? "ॲनेक्स्चर फाइल करणे" : "नवीन ॲनेक्स्चर फाइल करणे"}</h6>
                        </div>
                    </div>
                    <ul className="table-top-head">
                        <li>
                            <div className="page-btn">
                                <Link to={route.Annexure} className="btn btn-secondary">
                                    <ArrowLeft className="me-2" />
                                    ॲनेक्स्चर फाइल मास्टर वर परत जा
                                </Link>
                            </div>
                        </li>
                        <li>
                                            ...
                                            <label className="form-label">
                                                ...
                                                {formData.ALimage && (
                                                    <span
                                                        ...
                                                        onClick={() => openModalWithImage(formData.ALimage)}
                                                    >
                                                        {formData.ALimage.split("_").pop()}
                                                    </span>
                                                )}
                                            </label>
                                            <input
