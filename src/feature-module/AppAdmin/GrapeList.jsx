...
const GrapeList = () => {
    ...
    const [GrapesList, setAddList] = useState([]);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [previewFileUrl, setPreviewFileUrl] = useState(null);

    const onEditClick = (grid) => {
        navigate(route.AddGrapeList, { state: { grid: grid } });
    };

    const columns = [
        {
            title: "फोटो",
            dataIndex: "grapeimage",
            render: (fileName) => {
                if (!fileName) {
                ...
                <div className="action-table-data">
                    <div className="edit-delete-action">
                        <OverlayTrigger placement="top" overlay={<Tooltip id="edit-tooltip">Edit</Tooltip>}>
                            <a className="me-2 p-2" onClick={() => onEditClick(record.grid)}>
                        ...
                        <OverlayTrigger placement="top" overlay={<Tooltip id="delete-tooltip">Delete</Tooltip>}>
                            <Link
                                ...
                                to="#"
                                onClick={() => showConfirmationAlert(record.grid)}
                            >
    ...
    const OndeleteAccounts = async (grid) => {
        try {
            const payload = {
                grid: grid,
                companyid: "",
            ...
            const response = await axios.post(
                `${baseUrl.Url}/api/SP_DeleteGrapesList`,
                payload,
                { headers }
            );
            ...
            setAddList((prevState) => prevState.filter((item) => item.grid !== grid));
        } catch (error) {
    ...
    const showConfirmationAlert = (grid) => {
        MySwal.fire({
            title: "Are you sure?",
            ...
            <div className="content">
                <div className="page-header">
                    ...
                    <div className="page-btn">
                        <Link to={route.AddGrapeList} className="btn btn-added">
                            <PlusCircle className="me-2 iconsize" /> नवीन वूत्त
                        </Link>
                    </div>
                    <div className="page-btn">
                ...
                <div className="card table-list-card">
                    <div className="card-body">
                        <div className="table-responsive">
                            <Table columns={columns} dataSource={GrapesList} />
                        </div>
                    </div>
                </div>
