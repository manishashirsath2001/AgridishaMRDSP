...
const AddGrapeList = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                ...
                setFormData((prev) => ({ ...prev, GLimage: finalFileName }));
            } catch (error) {
                setFilePreviewUrl(null);
                setFormData((prev) => ({ ...prev, GLimage: "" }));
            } finally {
    ...
    const handleFormSubmission = async () => {
        try {
            const payload = {
                grid: grid ? grid : GUID,
                grapestitle: formData.Gtitle,
                grapeimage: formData.GLimage,
    ...
    const checkFormValidity = (e) => {
        const { Gtitle, GLimage, isactive } = formData;
        if (!Gtitle) {
            Swal.fire({
                icon: "error",
    ...
    useEffect(() => {
        ...
        const fetchMasterData = async () => {
            try {
                ...
                const response = await axios.post(
                    `${baseUrl.Url}/api/GET_GrapesList`,
                    payload,
                    { headers }
                );
                ...
                setFormData({
                    grid: apiData.grid || "",
                    Gtitle: apiData.grapestitle || "",
                    GLimage: apiData.grapeimage || "",
                ...
                if (apiData.GLimage) {
                    ...
                    const fileUrl = fileExtension === 'pdf'
                        ? `${baseUrl.Url}/Assets/${apiData.GLimage}`
                        : `${baseUrl.Url}/Images/${apiData.GLimage}`;
                    setFilePreviewUrl(fileUrl);
                }
            } catch (error) {
                ...
                <div className="page-header">
                    <div className="add-item d-flex">
                        <div className="page-title">
                            <h3>{grid ? "सूची" : "नवीन सूची"}</h3>
                            <h6>{grid ? "सूची करणे" : "नवीन सूची करणे"}</h6>
                        </div>
                    </div>
                                            ...
                                            <label className="form-label">
                                                ...
                                                {formData.GLimage && (
                                                    <span
                                                        ...
                                                        onClick={() => openModalWithImage(formData.GLimage)}
                                                    >
                                                        {formData.GLimage.split("_").pop()}
                                                    </span>
                                                )}
                                            </label>
                                            <input
