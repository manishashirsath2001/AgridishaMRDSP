...
const AddRateList = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        if (file) {
            ...
            try {
                ...
                setFormData((prev) => ({ ...prev, RLimage: finalFileName }));
            } catch (error) {
                setFilePreviewUrl(null);
                setFormData((prev) => ({ ...prev, RLimage: "" }));
            } finally {
    ...
    const handleFormSubmission = async () => {
        try {
            const payload = {
                rid: formData.rid || generatedID,
                ratetitle: formData.Rtitle,
                rLimage: formData.RLimage,
    ...
    const checkFormValidity = (e) => {
        const { Rtitle, RLimage, isactive } = formData;
        if (!Rtitle) {
            Swal.fire({
                icon: "error",
    ...
    useEffect(() => {
        ...
        const fetchMasterData = async () => {
            try {
                ...
                setFormData({
                    rid: apiData.rid || "",
                    Rtitle: apiData.ratetitle || "",
                    RLimage: apiData.rLimage || "",
                    Description: apiData.description || "",
                    isactive: String(apiData.isactive || ""), // Ensure string for dropdown
                });
                if (apiData.rLimage) {
                    const fileExtension = apiData.rLimage.split('.').pop().toLowerCase();
                    const fileUrl = fileExtension === 'pdf'
                        ? `${baseUrl.Url}/Assets/${apiData.rLimage}`
                        : `${baseUrl.Url}/Images/${apiData.rLimage}`;
                    setFilePreviewUrl(fileUrl);
                }
            } catch (error) {
                ...
                <div className="page-header">
                    <div className="add-item d-flex">
                        <div className="page-title">
                            <h3>{rid ? "भाव सूची" : "नवीन जाहिरात"}</h3>
                            <h6>{rid ? "भाव सूची करणे" : "नवीन जाहिरात करणे"}</h6>
                        </div>
                    </div>
                    <ul className="table-top-head">
                        <li>
                            <div className="page-btn">
                                <Link to={route.RateList} className="btn btn-secondary">
                                    <ArrowLeft className="me-2" />
                                    भाव सूची मास्टर वर परत जा
                                </Link>
                            </div>
                        </li>
                        <li>
                                            ...
                                            <label className="form-label">
                                                ...
                                                {formData.RLimage && (
                                                    <span
                                                        ...
                                                        onClick={() => openModalWithImage(formData.RLimage)}
                                                    >
                                                        {formData.RLimage.split("_").pop()}
                                                    </span>
                                                )}
                                            </label>
                                            <input
