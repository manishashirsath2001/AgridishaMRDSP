...
function AddPurchaseReturn({ PRAID }) {
    const { userdetail } = getUserData();
    const [showForm, setShowForm] = useState(false);
    const [PBBILLNO, setPBBILLNO] = useState();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                "pbrefewence": formData.PBAID ? formData.PBAID : formData.REFF,
            ...
            if (response1.status === 200) {
                ...
                if (response2.status === 200) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            const modal = document.getElementById("AddPurchaseReturn");
                            if (modal) {
    ...
    const validateinput = (e) => {
        ...
        if (
            (!BillNumber || !/^\d{4,}$/.test(BillNumber)) ||
    ...
    const handleSearch = async (PBBILLNO) => {
        if (PBBILLNO) {
            setShowForm(true);
            try {
                const payload = {
                ...
                const response = await axios.post(
                    `${baseUrl.Url}/backend/api/GET_PBillNoSearch`,
                    payload,
                    { headers }
                );
                if (response.status !== 200)
                ...
                if (response.data.length > 0) {
                    setFormData(prevState => ({
                        ...prevState,
                        BillNumber: response.data[0].pbbillno,
                        Consigner: response.data[0].pbconsigner,
                        Vendor: response.data[0].pbvendorid,
                        Vendorname: response.data[0].vendorname,
                        BillDate: convertToISODate(response.data[0].pbdate),
                        netAMT: response.data[0].pbnetamount,
                        PBAID: response.data[0].pbaid,
                        PMODE: response.data[0].paymentmode,
                    }));

                    const mappedProducts = response.data.map((item) => ({
                        ProductName: item.productname,
                        Product: item.pbproduct,
                        UOM: item.pbduom,
                        UOMtitle: item.uomtitle,
                        Quantity: item.pbdquantity,
                        Total: item.pbdtamt,
                        returnQuantity: 0,
                                                    ...
                                                    <div className="col-12 col-sm-10 col-md-8 col-lg-6 col-xl-4">
                                                        <div className="search-input d-flex align-items-center">
                                                            <input
                                                                ...
                                                                value={PBBILLNO}
                                                                onChange={(e) => setPBBILLNO(e.target.value)}
                                                                ref={BillRef}
                                                            />
                                                            <button
                                                                type="button"
                                                                onClick={() => handleSearch(PBBILLNO)}
                                                ...
                                                {showForm && (
                                                    <div>
                                                        ...
                                                        <div className="row">
                                                            ...
                                                            <div className="col-lg-3 col-md-6 col-sm-12">
                                                                <div className="input-blocks">
                                                                    <label className="form-label required">बिल तारीख</label>
                                                                    <div className="input-groupicon calender-input">
                                                                        {/* <Calendar className="info-img" /> */}
                                                                        <input
                                                                            type="date"
