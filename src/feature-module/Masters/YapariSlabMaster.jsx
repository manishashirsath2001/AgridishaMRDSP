...
const YapariSlabMaster = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const data = useSelector((state) => state.toggle_header);
    const route = all_routes;

    const [vyapariSlabMaster, setVyapariSlabMaster] = useState([]);
    ...
    // fetch VyapariSlabMaster Master Data 
    useEffect(() => {

        const fetchVyapariSlabMaster = async () => {
            try {
                const payload = {
                    "vsaid": "%",
                    "keyword": "%",
                ...
                const response = await axios.post(
                    `${baseUrl.Url}/backend/api/GET_VyapariSlabMaster`,
                    payload,
                    { headers }
                );
                if (response.status !== 200)
                    throw new Error("Failed to fetch ");
                console.log("GET_VyapariSlabMaster", response.data)
                setVyapariSlabMaster(response.data);
            } catch (error) {
                console.error("Error fetching GET_vyapariSlabMaster data:", error);
            }
        };

        fetchVyapariSlabMaster();
    ...
    const showConfirmationAlert = (vsaid) => {
        MySwal.fire({
            ...
            text: "ही क्रिया परत करता येणार नाही!",
    ...
    const exportToExcel = async () => {
        try {
            ...
            vyapariSlabMaster.forEach(({ schemeno, schemedate, schemename, description }) => {
    ...
    const generatePDF = (vyapariSlabMaster) => {
        const doc = new jsPDF();
        ...
        const tableRows = vyapariSlabMaster.map((item) => {
                ...
                <div className="page-header">
                    <div className="add-item d-flex">
                        <div className="page-title">
                            <h3>व्यापारी स्लॅब  </h3>
                            <h6>म्यानेज व्यापारी स्लॅब  </h6>
                        </div>
                    </div>
                ...
                <div className="card table-list-card">
                    <div className="card-body">
                        <div className="table-responsive">
                            <Table columns={columns} dataSource={vyapariSlabMaster} />
                        </div>
                    </div>
                </div>
...
export default YapariSlabMaster;
