...
const QuickFarmerBill = ({ BAID }) => {
    ...
    const fetchBillData = async () => {
        try {
            ...
            const response = await axios.post(
                `${baseUrl.Url}/backend/api/GET_CreateBill`,
            ...
            setfarmeName(response.data[0].fullname);
    ...
    const downloadExcel = async () => {
        try {
            ...
            const worksheet = workbook.addWorksheet("शेतकरी (Report)");

            // **Main Heading Row**
            const headingRow = worksheet.addRow(["शेतकरी (Report)"]);
            ...
            // **Header Row**
            const headers = ["तारीख", "बिल नं", "शेतकरी", "वजन", "भाव", "किमत ", "व्यापारी"];
            ...
            const columnWidths = [25, 25, 25, 25, 25, 25, 25,]; // Adjust widths as needed
            columnWidths.forEach((width, index) => {
                worksheet.getColumn(index + 1).width = width;
            });

            // **Adding Data Rows**
            tabledata.forEach(({ date, billno, fullname, weight, rate, amount, vname }) => {
                const row = worksheet.addRow([date, billno, fullname, weight, rate, amount, vname]);
                row.eachCell((cell) => {
                    cell.alignment = { horizontal: "center", vertical: "middle" };
            ...
            saveAs(data, "शेतकरी (Report).xlsx");
                    ...
                    {/* <div className="row align-items-start pos-wrapper">
                        ...
                        <div className="col-md-12 col-lg-4 ps-0">
                            <aside className="product-order-list">
                                ...
                                <div className="btn-row d-sm-flex  ">
                                    <Link
                                        to="#"
                                        className="btn btn-secondary btn-icon flex-fill"
                            ...
                            <table className="table-borderless w-100 table-fit">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="text-center">Vyapari</th>
                                    ...
                                    {tabledata.length > 0 ? (
                                        tabledata.map((row, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="text-center">{row.vname}</td>
...
export default QuickFarmerBill
